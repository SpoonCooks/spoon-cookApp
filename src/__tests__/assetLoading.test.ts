...
describe('the native splash background', () => {
   ...
   *
   * Fixing that by pointing all three slots at the raw brand export traded one fault for two.
   * `spoon-brand-logo.png` carries its mark in the TOP HALF -- 739px above the centre of a 4096
   * canvas -- with no safe-zone margin, so Android scaled the lot to 108dp and drew the logo high
   * and oversized in its tile. And as a NOTIFICATION icon a full-colour file is worse than wrong:
   * Android keeps only the alpha and fills the shape with the accent, so a coloured logo arrives
   * as a solid blob. Each slot now gets the file built for it.
   */
  it('gives each icon slot the file built for it', () => {
    // Generated by ../spoon-frontend/scripts/build-app-icons.py: ink-centred, safe-zone sized.
    expect(config).toMatch(/^  icon: '\.\/assets\/images\/app-icon\.png'/m);
    expect(config).toContain("foregroundImage: './assets/images/android-icon-foreground.png'");
    // Themed icons tint an alpha shape; without this Android shrinks the colour icon into the slot.
    expect(config).toContain("monochromeImage: './assets/images/android-icon-monochrome.png'");

    // The raw export must not be feeding any icon slot again.
    const iconSlots = config.slice(0, config.indexOf('plugins:'));
    expect(iconSlots).not.toContain('figma-v13/spoon-brand-logo.png');
  });

  it('hands the notification tray an alpha silhouette, not a colour logo', () => {
    const notifications = config.slice(config.indexOf("'expo-notifications'"));
    const block = notifications.slice(0, 900);

    expect(block).toContain("icon: './assets/images/android-icon-monochrome.png'");
    expect(block).toContain('color: BRAND_YELLOW');
    // The exact file that used to be here, and the reason this test exists.
    expect(block).not.toContain('spoon-brand-logo.png');
  });
