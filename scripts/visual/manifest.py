...
| diff %      | `differingPixelPercent` at the antialiasing tolerance                          |
| shift       | `displacementProbe.bestVerticalOffsetPx`                                       |
...
`shift` sits next to `diff %` on purpose. A low percentage on a displaced screen is not a pass —
rasterisation noise and a shifted layout produce similar percentages and are entirely different
defects — so the two columns are read together.
...
def render(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        shift = f"{r['shift']:+d}" if r["shift"] is not None else "—"
        ...
        lines.append(
            ...
            f"{diff} | {shift} | {unmatched} | {r['verdict']} |"
        )
    ...
    lines += [
        ...
        f"- SCREENS_ON_THEIR_DESIGN_ROW: {sum(1 for r in rows if r['shift'] == 0)}",
