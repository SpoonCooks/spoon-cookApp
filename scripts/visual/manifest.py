...
"Implemented" deliberately requires **both** a gallery entry and a captured design context. Most screens carry a
gallery entry inherited from the V13 build; they render, but they were never built against a V14
`get_design_context`, and V14 changed the frame under many of them (a 68-unit bottom nav on 33 of
47). Counting those as implemented would overstate progress against V14, so they are reported as
`pre-V14` instead.
...
def render(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        if r["v13Implemented"]:
            ...
            implemented = f"pre-V14 (`{r['galleryState']}`)"
        else:
            implemented = "—"
    ...
    lines += [
        ...
        f"- SCREENS_WITH_A_GALLERY_STATE_BUT_NO_V14_CONTEXT: "
